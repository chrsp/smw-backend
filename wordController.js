// wordController.js

Word = require('./wordModel');

exports.index = function (req, res) {
    Word.get(function (err, contacts) {
        if (err) {
            res.json({
                status: "error",
                message: err,
            });
        }
        res.json({
            status: "success",
            data: contacts
        });
    });
};

exports.new = function (req, res) {
    var word = new Word();
    word.desc = req.body.desc ? req.body.desc : word.desc;
    
    word.save(function (err) {
        // if (err)
        //     res.json(err);
        res.json({
            message: 'New word created!',
            data: word
        });
    });
};

exports.view = function (req, res) {
    Word.findById(req.params.contact_id, function (err, word) {
        if (err)
            res.send(err);
        res.json({
            message: 'Word details loading..',
            data: word
        });
    });
};

exports.update = function (req, res) {
    Word.findById(req.params.contact_id, function (err, word) {
            if (err)
                res.send(err);
    word.description = req.body.description ? req.body.description : word.description;
    
            word.save(function (err) {
                if (err)
                    res.json(err);
                res.json({
                    message: 'Word Info updated',
                    data: word
                });
            });
        });
    };
    
    exports.delete = function (req, res) {
        Word.remove({
            _id: req.params.word_id
        }, function (err, word) {
            if (err)
                res.send(err);
    res.json({
                status: "success",
                message: 'Word deleted'
            });
        });
    };